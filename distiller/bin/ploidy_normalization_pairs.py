...
### A .pairs file has at least the following 5 columns: readID  chr1    pos1    chr2    pos2
### The script duplicates each line (i.e. each interaction) and split the interactions coming from the cores in
...
### The best way to display heatmaps from the ploidy-corrected .pairs file is to separate the 2 haplotipes.
import sys
...
with open(sys.argv[1]) as in_file:
    ...
    for line in in_file.readlines():
        ...
        read_name, chr_1, pos_1, chr_2, pos_2, *extra = line[:-1].split()
        # both contigs diploid: -> split into A and B
        if chr_1 in DIPLOID_CONTIGS and chr_2 in DIPLOID_CONTIGS:
            # print line for A
            print(read_name + ":0", DIPLOID_A_CONTIG_NAMES[chr_1], pos_1, 
                  DIPLOID_A_CONTIG_NAMES[chr_2], pos_2, *extra, sep="\t")
            # print line for B
            print(read_name + ":1", DIPLOID_B_CONTIG_NAMES[chr_1], pos_1, 
                  DIPLOID_B_CONTIG_NAMES[chr_2], pos_2, *extra, sep="\t")
        
        # neither diploid: -> print twice
        elif chr_1 not in DIPLOID_CONTIGS and chr_2 not in DIPLOID_CONTIGS:
            # print line twice
            for idx in range(2):
                print(read_name + ":" + str(idx), chr_1, pos_1, chr_2, pos_2, *extra, sep="\t")
